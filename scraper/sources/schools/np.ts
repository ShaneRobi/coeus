...
export class NPScraper extends BaseScraper {
  readonly source = 'np'
  ...
  async scrape(): Promise<ScrapedEvent[]> {
    ...
    try {
      ...
      await page.goto('https://www.np.edu.sg/news-and-events/events', { waitUntil: 'domcontentloaded', timeout: 30000 })
      ...
      for (const item of items) {
        ...
        events.push({
          ...
          location_name: item.venue || 'NP',
          location_address: item.venue ? `${item.venue}, Ngee Ann Polytechnic, Singapore` : 'Ngee Ann Polytechnic, Singapore',
