...
export class SITScraper extends BaseScraper {
  readonly source = 'sit'
  ...
  async scrape(): Promise<ScrapedEvent[]> {
    ...
    try {
      ...
      await page.goto('https://www.singaporetech.edu.sg/events', { waitUntil: 'domcontentloaded', timeout: 30000 })
      const items = await page.$$eval('[class*="event"], [class*="Event"], .views-row, article', (els) =>
        els.slice(0, 30).map((el) => ({
          ...
          date: el.querySelector('time')?.getAttribute('datetime')
            ?? el.querySelector('time, [class*="date"], [class*="Date"]')?.textContent?.trim()
            ?? '',
      ...
      for (const item of items) {
        ...
        events.push({
          ...
          location_name: item.venue || 'SIT',
          location_address: item.venue ? `${item.venue}, Singapore Institute of Technology, Singapore` : 'Singapore Institute of Technology, Singapore',
